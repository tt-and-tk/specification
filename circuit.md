QurgeのCPUを含む実際の回路を構成するファイルについて説明する．シミュレーション専用のファイル(テストベンチやモデル)は対象外とする．

# ファイル種別
## svh
svファイルで使用する型・定数・enum・関数・interface等の定義をまとめたファイル群であり，必要とするsvファイルがある場合にのみ存在する(複数のsvファイルから共有されることもある)．

- util 特定の回路に属さない汎用の型・定数・関数を定義
- ram メインメモリに関する型・定数と，その読み書き用のインターフェースを定義
- machine 機械語とその各フィールドの型・命令コード定数と，オペランドを受け取って各命令の機械語を組み立てる関数を定義
- rom ROMに関する型・定数と，その読み込み用のインターフェースを定義
- decoder 機械語とそのデコード結果を受け渡すインターフェースを定義
- alu ALUに関する型・定数と，レジスタの読み書き可否や命令の実行可否を判定する関数を定義
