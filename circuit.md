...
- util 特に分類できない
- ram メインメモリのアドレス・データのバス幅の型・容量の定数・アクセス結果のenumと，読み込み・書き込みインターフェース(ram_read_if/ram_write_if)を定義
- machine 機械語を生成する関数群を定義
- rom ROMのプログラムカウンタの型・最大命令数の定数と，読み込みインターフェース(rom_read_if)を定義
- decoder 機械語と，それを分解した各フィールド(命令タイプ・func・mask・レジスタ番号・イミディエイト)を受け渡すインターフェース(command_if)を定義
- alu aluに関するデータ型宣言など
...
- seven_seg デバッグ用の7セグメントディスプレイ表示モジュール．Pynthesis本体とは無関係な開発時限定の使用
