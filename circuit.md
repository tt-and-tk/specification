...
- seven_seg 7セグメントディスプレイのセグメントパターン変換モジュール．デバッグ用であり，自作PC本体とは無関係な開発時限定の使用．svファイルを内包しない独立したモジュール定義そのもの．

# 階層構造
sv・vファイル群のインスタンス化の親子関係(svhはインスタンス化されないため対象外)．ブロックデザインに直接組み込まれるのはmother_board(v)とseven_seg(v)で，他のsvモジュールはmother_boardの内部で間接的にインスタンス化される．seven_segは他のどのモジュールともインスタンス化関係を持たない独立した葉ノード．
