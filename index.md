...
| Qosmos(コスモス) | 自作OS．現在はROM上で動くシェル・ファイルシステム・SDカードドライバの一式 | pynq + 「秩序ある世界」を意味するギリシャ語(kosmos)．Demiurgeが混沌から秩序ある世界を形作ることにちなみ，Qurgeが動かす世界として同じくpynqのqを冠する |
...
1. [シェル](./shell.md) — シェルの起動・入力解析・コマンド一覧・実行ファイルの実行・SDカード上のFAT32ファイルシステムの読み書き
